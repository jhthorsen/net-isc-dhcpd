...
has arguments => (
    ...
    default => sub {
        {
        port      => { "text" => "port %s", regex => qr/^ \s+ port \s+ (\d+)\s*;/x },
        peer_port => { "text" => "peer port %s", regex => qr/^ \s+ peer \s+ port \s+ (\d+)\s*;/x },
        address   => { "text" => "address %s", regex => qr/^ \s+ address \s+ (\S+)\s*;/x },
        peer_address => { "text" => "peer address %s", regex => qr/^ \s+ peer \s+ address \s+ (\S+)\s*;/x },
        type => { "text" => "%s", regex => qr/^ \s+ (primary|secondary)\s*;/x },
        max_response_delay => { "text" => "max-response-delay %s", regex => qr/^ \s+ max-response-delay \s+ (\d+)\s*;/x },
        max_unacked_updates => { "text" => "max-unacked-updates %s", regex => qr/^ \s+ max-unacked-updates \s+ (\d+)\s*;/x },
        lb_max_seconds => { "text" => "load balance max seconds %s", regex => qr/^ \s+ load\s+balance\s+max\s+seconds \s+ (\d+)\s*;/x },
        mclt => { "text" => "mclt %s", regex => qr/^ \s+ mclt \s+ (\d+)\s*;/x },
        split => { "text" => "split %s", regex => qr/^ \s+ split \s+ (\d+)\s*;/x },
        auto_partner_down => { "text" => "auto-partner-down %s", regex => qr/^ \s+ auto-partner-down \s+ (\d+)\s*;/x },
        max_lease_misbalance => { "text" => "max-lease-misbalance %s", regex => qr/^ \s+ max-lease-misbalance \s+ (\d+)\s*;/x },
